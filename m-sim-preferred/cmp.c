...
: id(rhs.id), max_contexts(rhs.max_contexts), power(rhs.power),
	...
	sim_num_loads(rhs.sim_num_loads), sim_total_loads(rhs.sim_total_loads), sim_num_branches(rhs.sim_num_branches), sim_total_branches(rhs.sim_total_branches), l1_hits_passedthrough(rhs.l1_hits_passedthrough),
...
void core_t::print_stats(FILE * stream, counter_t sim_cycle)
{
	...
	fprintf(stream, "l1_hits_passedthrough_%d			%lld # total number of l1 hits that were passed through at the write buffer during the commit stage\n", id, l1_hits_passedthrough);
}
